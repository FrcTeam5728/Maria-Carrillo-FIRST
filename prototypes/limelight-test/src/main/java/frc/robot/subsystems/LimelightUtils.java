...
public class LimelightUtils {
    ...
    private NetworkTableEntry tx;  // Horizontal offset from crosshair to target (-27 to 27 degrees)
    private NetworkTableEntry ty;  // Vertical offset from crosshair to target (-20.5 to 20.5 degrees)
    private NetworkTableEntry ta;  // Target area (0% to 100% of image)
    private NetworkTableEntry tid; // Target ID (which AprilTag)
    
    // 3D position data (if available)
    private NetworkTableEntry botpose; // Robot's position in field space
    private NetworkTableEntry targetpose; // Target's position in camera space
    ...
    public LimelightUtils(String tableName) {
